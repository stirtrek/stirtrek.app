...
function scoreLabel(session: SessionFeedbackSummary): string {
  ...
  // Score: green=3, yellow=2, red=1, normalized to percentage
  const score =
...
export default function AdminFeedbackPage() {
  ...
  useEffect(() => {
    fetch(`/${eventSlug}/api/admin/feedback`)
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Tap a session to see individual feedback. Score is based on green (3),
        yellow (2), red (1) normalized to a percentage.
      </p>
                ...
                return (
                  <div key={session.id} className="py-2">
                        ...
                        {session.total > 0 && (
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-[10px] text-muted-foreground">
                              {session.total} response{session.total !== 1 && "s"} · Score: {scoreLabel(session)}
                            </span>
                          </div>
                        )}
                        {session.total === 0 && (
                    ...
                    {isExpanded && session.feedback.length > 0 && (
                      <div className="mt-2 space-y-2 pl-2 border-l-2 border-muted ml-1">
