...
export default function AdminSponsorsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`/${eventSlug}/api/admin/sponsor-accounts`);
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      const res = await fetch(`/${eventSlug}/api/admin/sponsors/sync`, { method: "POST" });
      const data = await res.json();
  ...
  const handleExport = (sponsorId: string, sponsorName: string) => {
    window.open(
      `/${eventSlug}/api/admin/sponsor-accounts/export?sponsor_id=${sponsorId}`,
      "_blank"
    );
